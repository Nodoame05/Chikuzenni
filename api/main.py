...
def teacher_all(uuid):
    ...
    if token != teacher_data.get("token"):
        return jsonify({"message":"アクセスが拒否されました。"}),401
    return jsonify({"name":teacher_data.get("name"),"email":teacher_data.get("email")}),200
...
def student_all(uuid):
    ...
    if token != student_data.get("token"):
        return jsonify({"message":"アクセスが拒否されました。"}),401
    return jsonify({"name":student_data.get("name"),"email":student_data.get("email")}),200
...
def teacher_login():
    ...
    return jsonify({"message":"success","token":teacher_token,"uuid":teacher_data.get("uuid")}),200
...
def change_status_names(uuid):
    ...
    if teacher_status_list.get("3") == None or teacher_status_list.get("4") == None or teacher_status_list.get("5") == None:
...
def student_login():
    ...
    return jsonify({"message":"success","token":student_token,"uuid":student_data.get("uuid")}),200
